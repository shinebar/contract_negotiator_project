export async function propose_terms({ terms }) {
  console.log(`Negotiating Agent Party B: Proposing terms: ${terms}`);
...
propose_terms.input_schema = {
  type: "object",
  properties: {
    terms: { type: "string", description: "The terms being proposed by Party B" },
  },
  required: ["terms"],
};
...
export async function accept_terms({ terms }) {
  console.log(`Negotiating Agent Party B: Accepting terms: ${terms}`);
...
accept_terms.input_schema = {
  type: "object",
  properties: {
    terms: { type: "string", description: "The terms being accepted by Party B" },
  },
  required: ["terms"],
};
...
export async function reject_terms({ reason }) {
  console.log(`Negotiating Agent Party B: Rejecting terms due to: ${reason}`);
